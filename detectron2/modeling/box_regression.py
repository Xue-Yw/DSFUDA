...
def _dense_box_regression_loss(
    ...
    """
    anchors = type(anchors[0]).cat(anchors).tensor  # (R, 4)## 将锚点连接成一个张量 (形状: (R, 4))
    
    if box_reg_loss_type == "smooth_l1":
        gt_anchor_deltas = [box2box_transform.get_deltas(anchors, k) for k in gt_boxes]# 计算每个gt_boxes相对于锚点的偏移量
        gt_anchor_deltas = torch.stack(gt_anchor_deltas)  # (N, R, 4)堆叠偏移量张量 
        loss_box_reg = smooth_l1_loss(
            cat(pred_anchor_deltas, dim=1)[fg_mask],# # 获取前景锚点的预测偏移量
            gt_anchor_deltas[fg_mask],# 获取前景锚点的真实偏移量
            beta=smooth_l1_beta,
            reduction="sum",
        )
