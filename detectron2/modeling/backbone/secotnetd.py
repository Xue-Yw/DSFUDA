...
__all__ = [
    "SEDCANetDBlockBase",
    "SEDCANetD",
    "make_sedcanetd_stage",
    "build_sedcanetd_backbone",
]
...
class DCALayer(nn.Module):
    def __init__(self, dim, kernel_size, norm=None):
        super(DCALayer, self).__init__()
    ...
    def __init__(
        ...
        *,
        ...
        if (bottleneck_channels in res2_3_dim) or (bottleneck_channels == 256 and block_idx % 2 != 0):
          ...
          self.conv2 = DCALayer(bottleneck_channels, kernel_size=3, norm=norm)
          if stride_3x3 > 1:
...
SEDCANetDBlockBase = CNNBlockBase

def make_sedcanetd_stage(*args, **kwargs):
    """
    Deprecated alias for backward compatibiltiy.
    """
    return SEDCANetD.make_stage(*args, **kwargs)


def build_sedcanetd_backbone(cfg, input_shape):
    ...
    for idx, stage_idx in enumerate(range(2, max_stage_idx + 1)):
        ...
        #    else:
        #        stage_kargs["block_class"] = DCABlock
        stage_kargs["bottleneck_channels"] = bottleneck_channels
        stage_kargs["stride_in_1x1"] = stride_in_1x1
        stage_kargs["dilation"] = dilation
        stage_kargs["num_groups"] = num_groups
        ...
        blocks = SEDCANetD.make_stage(**stage_kargs)
        in_channels = out_channels
        out_channels *= 2
        bottleneck_channels *= 2
        stages.append(blocks)
    return SEDCANetD(stem, stages, out_features=out_features).freeze(freeze_at)
