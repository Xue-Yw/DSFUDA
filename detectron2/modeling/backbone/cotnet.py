...
class DCALayer(nn.Module):
    def __init__(self, dim, kernel_size, norm=None):
        super(DCALayerLayer, self).__init__()
        ...
        if self.dim >= 128:
            print(dim)
            self.key_embed = nn.Sequential(
                DeformConv2d(dim, dim, self.kernel_size, padding=self.kernel_size // 2, stride=1, bias=None, modulation=False),
...
DCANetBlockBase = CNNBlockBase

def make_stage(*args, **kwargs):
    """
    Deprecated alias for backward compatibiltiy.
    """
    return DCANet.make_stage(*args, **kwargs)


@BACKBONE_REGISTRY.register()
def build_dcanet_backbone(cfg, input_shape):
    ...
    for idx, stage_idx in enumerate(range(2, max_stage_idx + 1)):
        ...
        blocks = DCANet.make_stage(**stage_kargs)
        in_channels = out_channels
        out_channels *= 2
        bottleneck_channels *= 2
        stages.append(blocks)
    return DCANet(stem, stages, out_features=out_features).freeze(freeze_at)
